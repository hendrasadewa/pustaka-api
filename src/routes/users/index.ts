...
import { assertUserExists, assertUserActive, type UserEntity } from "./entity";
import {
  RegisterSchema,
  LoginSchema,
...
import { RESET_TOKEN_TTL, SESSION_TOKEN_TTL } from "../../configs/jwt-configs";
import { currentTimestamp, generateExpiryTime } from "../../utils/time";

const userApp = new Hono<{ Bindings: Env }>();

userApp
  .get("/me", jwtGuardMiddleware, async (ctx) => {
  ...
  .post("/login", zValidator("json", LoginSchema), async (ctx) => {
    ...
    if (!authData) {
      return ctx.json(fail("Invalid email or password"), 401);
    }
  ...
  .patch("/:id/deactivate", jwtGuardMiddleware, async (ctx) => {
    const id = parseId(ctx.req.param("id"));
    const payload = ctx.get("jwtPayload") as { sub: number };
